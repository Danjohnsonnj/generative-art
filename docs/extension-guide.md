...
- **Phase 6 — IndexedDB:** `js/core/store-indexeddb.js` provides isolated production/test databases and atomic create/list/load/update/delete storage for work metadata, drafts, and immutable revisions. Its browser test setup always deletes `generative-art-canvas-v1-test`, never the production `generative-art-canvas-v1` library. `createRevisionThumbnail()` in `js/core/thumbnails.js` resolves a checkpoint, renders a 192px-wide PNG, and returns a cloned revision with its `thumbnail` data URL record; include that returned revision in the snapshot saved through `updateWork()`. The Library / WIP panel opens, renames, and confirms deletion of stored works while showing draft-vs-saved state. Malformed JSON import fails with an actionable message and leaves the library unchanged. **Live as of Phase 6 close (2026-07-11).**
- **Phase 7 — SVG serialization:** for a `capabilities.svg: true` style, confirm `renderSVG` produces a valid, non-empty SVG string with no `DOMParser` errors; reopen the exported SVG file in a browser and compare its composition to the screen. Confirm SVG export is disabled with an explanation for `canvas`-only styles. **Live as of Phase 7 close (2026-07-11).**
