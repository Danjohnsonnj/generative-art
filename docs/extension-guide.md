...
- **Phase 6 — IndexedDB:** `js/core/store-indexeddb.js` now provides isolated production/test databases and atomic create/list/load/update/delete storage for work metadata, drafts, and immutable revisions. Its browser test setup always deletes `generative-art-canvas-v1-test`, never the production `generative-art-canvas-v1` library. `createRevisionThumbnail()` in `js/core/thumbnails.js` resolves a checkpoint, renders a 192px-wide PNG, and returns a cloned revision with its `thumbnail` data URL record; include that returned revision in the snapshot saved through `updateWork()`. After the Slice 6.3 library UI lands, confirm persisted works reopen after a page reload without JSON import and deleting one work leaves others intact.
